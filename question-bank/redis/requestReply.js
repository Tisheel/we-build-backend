...
sub.pSubscribe('REQ_QB:*', async (message, channel) => {
    ...
    try {
        ...
        const question = await QuestionBank.find(filter)

        pub.publish(`RPLY_QB:${channelId}`, JSON.stringify(question))

    } catch (error) {

        pub.publish(`RPLY_QB:${channelId}`, "0")
        console.log(error)
